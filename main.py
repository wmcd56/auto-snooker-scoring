...
def main():
    ...
    while True:
        ...
        for i in range(len(colour_circles)):
            ...
            # uncomment below for multiprocessing ball tracking
            # with concurrent.futures.ProcessPoolExecutor() as executor:
            #     # uncomment below for sequential ball tracking
            #     for ball in zip(frame.balls, executor.map(frame.balls.ball.track_ball,
            #                                               (frame.balls.ball.loc, frame.balls.ball.colour))):
            #         print(f"Ball: {ball}")
            for ball in frame.balls:
                if ball.colour == ball_colour and ball.colour != 'red':
                    ball.track_ball(location, ball_colour)
                # The following code segment draws a line behind the ball which shows it's path

                for i in range(1, len(ball.loc)):
                    # print(f'{ball.colour} ball loc {ball.loc[i]}')
                    # if either of the tracked points are None, ignore
                    # them
                    # if ball.loc[i - 1] is None or ball.loc[i] is None:
                        # continue
        ...
        if fouls_detected is False:
            ...
            for ball in frame.balls:
                if ball.colour != 'red':
                    if ball.colour not in latest_ball_colours and ball.colour not in temp_ball_colours:  # and
                        additional_points += Ball.colour_point_list[ball.colour]
                elif ball.colour == 'red':
                    frame_reds += 1
                i += 1
            for colour in latest_ball_colours:
                if colour == 'red':
                    num_reds += 1
